  ...
  return (
    <div className="names-list">
      ...
      <button  class="step1b"> 3 </button>
        ...
        <h className="titlebv">نوع خرید</h>
        <div className="names">
        
        {names.map((name) => (
          <div className='namesindivi' key={name.id}>
          <div  className={`namei ${name.selected ? 'selected' : ''}`} onClick={() => handleNameClick(name.id)}>
            {name.name}
            </div>
      ...
      <div className="add-name">
        ...
        />
        <button onClick={handleAddName}>Add Category</button> 
      </div>
